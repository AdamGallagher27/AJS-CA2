...
const update = () => {
  ...
  useEffect(() => {
    ...
    const fetchRooms = async (surgery: Surgery) => {

      let rooms

      if (token) {
        rooms = await fetchAll('rooms', token)
        setAllRooms(rooms)
      }

      if (surgery.room) {
        setSelectedRoom(surgery.room._id)
      }
    }

    const fetchPatients = async (surgery: Surgery) => {

      let patients

      if (token) {
        patients = await fetchAll('patients', token)
        setAllPatients(patients)
      }

      if (surgery.patient) {
        setSelectedPatient(surgery.patient._id)
      }
    }

    const fetchWorkers = async (surgery: Surgery) => {

      let workers

      if (token) {
        workers = await fetchAll('workers', token)
        setAllWorkers(workers)
      }

      if (surgery.workers) {
        setSelectedWorkers(surgery.workers)
      }
    }
    ...
    const fetchSurgery = async () => {
      ...
      if (token) {
        surgery = await fetchById('surgeries', id, token)
        setForm(surgery)

        fetchRooms(surgery)
        fetchWorkers(surgery)
        fetchPatients(surgery)
      }
    }
  ...
  const update = async () => {
    ...
    try {
      const response = await axios.put(`${process.env.EXPO_PUBLIC_API_URL}/api/surgeries/${id}`, body,
        {
          headers: {
      ...
      if (response && response.data) {
        ...
        const pathToShow = `/resources/surgeries/${resolved._id}/show`
  ...
  return (
    <View style={styles.container}>
      <ScrollView>
        <Text style={styles.header}>Update New Surgery</Text>
          ...
          <List.Accordion title='Select Workers'>
            {allWorkers.map((worker) => (
              <View key={worker._id} style={styles.workerRow}>
                ...
                />
                <Text>{`${worker.worker_role} - ${worker.last_name}`}</Text>
              </View>
            ))}
          </List.Accordion>
        )}
